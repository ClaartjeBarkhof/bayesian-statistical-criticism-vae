...
def compute_all_divergences_data_model_groups(dp_mixtures, surprisal_values):
    ...
    for stat, dp_mixture in dp_mixtures.items():
        kl_density_est = estimate_kl_densities_dp_mixture(dp_mixture)
...
def plot_divergences_data_model_groups(all_df, sort_on="kl_comp sum", plot_only=None):
    ...
    if plot_only is None:
        ...
        df_select.drop("kl_comp sum", axis=1).plot.barh(rot=0, lw=0, ax=axs[0])
        axs[0].set_title("kl components")

        df_select = all_df[kl_dens_cols]
        df_select.drop("kl_dens sum", axis=1).plot.barh(rot=0, lw=0, ax=axs[1])
        axs[1].set_title("kl densities")
        axs[1].set_yticks([])

        df_select = all_df[was_cols]
        df_select.drop("wasserstein sum", axis=1).plot.barh(rot=0, lw=0, ax=axs[2])
        axs[2].set_title("wasserstein distances")
        axs[2].set_yticks([])

    elif plot_only == "kl_comp":
        df_select = all_df[kl_comp_cols]
        df_select.drop("kl_comp sum", axis=1).plot.barh(rot=0, lw=0)
        plt.title("kl components")

    elif plot_only == "kl_dens":
        df_select = all_df[kl_dens_cols]
        df_select.drop("kl_dens sum", axis=1).plot.barh(rot=0, lw=0)
        plt.title("kl density")

    elif plot_only == "wasserstein":
        df_select = all_df[was_cols]
        df_select.drop("wasserstein sum", axis=1).plot.barh(rot=0, lw=0)
