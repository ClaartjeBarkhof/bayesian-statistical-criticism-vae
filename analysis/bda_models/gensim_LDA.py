...
class LdaModel(interfaces.TransformationABC, basemodel.BaseTopicModel):
    ...
    def bound2(self, corpus, gamma=None, subsample_ratio=1.0):
        """Estimate the variational bound of documents from the corpus as E_q[log p(corpus)] - E_q[log q(corpus)].
        Parameters
        ----------
        corpus : iterable of list of (int, float), optional
        ...
        for d, doc in enumerate(corpus):  # stream the input doc-by-doc, in case it's too large to fit in RAM
            ...
            if gamma is None:
                gammad, _ = self.inference([doc])
                #print("new inference gammad.shape", gammad.shape)
            else:
                gammad = gamma[d][None, :]
                #print("given gamma gammad.shape", gammad.shape)
            Elogthetad = dirichlet_expectation(gammad)
            ...
            # E[log p(doc | theta, beta)]
            doc_score += sum(cnt * logsumexp(Elogthetad + Elogbeta[:, int(id)]) for id, cnt in doc)

            # E[log p(theta | alpha) - log q(theta | gamma)]; assumes alpha is a vector
            doc_score += np.sum((self.alpha - gammad) * Elogthetad)
            doc_score += np.sum((gammad) - gammaln(self.alpha))
            doc_score += gammaln(np.sum(self.alpha)) - gammaln(np.sum(gammad))
        ...
        # # Compensate likelihood for when `corpus` above is only a sample of the whole corpus. This ensures
        # # that the likelihood is always roughly on the same scale.
        # #total_score *= subsample_ratio
        #
        # # E[log p(beta | eta) - log q (beta | lambda)]; assumes eta is a scalar
        # total_score += np.sum((self.eta - _lambda) * Elogbeta)
        # total_score += np.sum(gammaln(_lambda) - gammaln(self.eta))
        #
        # scores += ((1/N) * np.sum((self.eta - _lambda) * Elogbeta))
        # scores += ((1/N) * np.sum(gammaln(_lambda) - gammaln(self.eta)))
        #
        # if np.ndim(self.eta) == 0:
        #     sum_eta = self.eta * self.num_terms
        # else:
        #     sum_eta = np.sum(self.eta)
        #
        # total_score += np.sum(gammaln(sum_eta) - gammaln(np.sum(_lambda, 1)))
        # scores += ((1/N) * np.sum(gammaln(sum_eta) - gammaln(np.sum(_lambda, 1))))

        # assert sum(scores) == total_score, "sum(scores) must be equal to total_score"
