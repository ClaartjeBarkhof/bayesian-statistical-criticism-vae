    ...
    for phase, data_loader in data_loaders.items():
        for i, (x_in, y) in enumerate(data_loader):
            ...
            conditional_sample_x = vae_model.gen_model.sample_generative_model(z=z_post,
                                                                               ...
                                                                               device="cuda:0").squeeze(0).squeeze(
                0)
...
def language_conditional_sample(data_loaders, vae_model, tok, device, conditional_sample_file, n_sample_batches=10, include_train=True):
    phases = ["train", "valid", "test"] if include_train else ["test", "valid"]
...
def encode_sample_all(df, device="cuda:0", include_train=True, n_sample_batches=20, batch_size=128,
                      sample_batch_size=100, tok=None):
...
def surprisal_data(surprisal_data_file, data_loaders, vae_model, n_iw_samples, max_batches=20):
    print("Data")
    d = dict()
    for phase, data_loader in data_loaders.items():
        ...
        # https://github.com/tom-pelsmaeker/deep-generative-lm/blob/c65bdf9d72e7d9d4e02576b1e84bce623725a0cd/util/evaluation.py#L200
        # Compute perplexity per word
...
def gather_surprisal_stats(run_df, device="cuda:0", include_train=True,
...
def evaluate(df, include_train=True, n_samples_mmd=5000, num_workers=3, batch_size=256, device="cuda:0"):
