...
def estimate_log_p_x(length_model, lengths, posterior_samples=None):
    if posterior_samples is None:
        posterior_samples = length_model.draw_posterior_predictions()
...
def estimate_log_p_x_conditional(length_model, lengths, local_samples=None):
    if local_samples is None:
        local_samples = length_model.infer_local_assignments(length_model.posterior_samples, length_model.y)
    ...
    assert len(lengths) == N_group_validation, "lengths must have the same length as the number of posterior rates"
