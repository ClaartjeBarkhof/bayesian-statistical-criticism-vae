...
class gated_resnet(nn.Module):
    def __init__(self, num_filters, conv_op, nonlinearity=concat_elu, skip_connection=0,
                 dim_i=28, conditional=False, h_dim=20):
        ...
        self.dim_i = dim_i
        if self.conditional:
            self.conditional_projection = nn.Linear(in_features=h_dim, out_features=dim_i * dim_i * 2)
    ...
    def forward(self, og_x, a=None, h=None):
        ...
        # Claartje: here the actual conditioning happens as proposed in
        # Van Oord et al. (2016) [https://arxiv.org/pdf/1606.05328.pdf]

        # here x is of size [B, F*2, dim_i, dim_i]
        # (it has been chunked along filter dim, so 2 the number of filters before chunking)
        # a and b are thus both of shape [B, F, dim_i, dim_i]
        # where dim_i is the current spatial dimensionality of x
        # (downsampled or not, depending on the layer)
        if self.conditional:
            # [B, Dim_i * Dim_i * 2]
            h_proj = self.conditional_projection(h)
            B = h_proj.shape[0]
