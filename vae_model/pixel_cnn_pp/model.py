from vae_model.pixel_cnn_pp.layers import *
from vae_model.pixel_cnn_pp.pixel_cnn_pp_utils import *
import numpy as np

class PixelCNNLayer_up(nn.Module):
    def __init__(self, nr_resnet, nr_filters, resnet_nonlinearity, dim_i=28, conditional=False, h_dim=20):
        super(PixelCNNLayer_up, self).__init__()
        self.nr_resnet = nr_resnet
        # stream from pixels above
        self.u_stream = nn.ModuleList([gated_resnet(nr_filters, down_shifted_conv2d,
                                                    resnet_nonlinearity, skip_connection=0, dim_i=dim_i,
                                                    conditional=conditional, h_dim=h_dim)
                                       for _ in range(nr_resnet)])

        # stream from pixels above and to thes left
        self.ul_stream = nn.ModuleList([gated_resnet(nr_filters, down_right_shifted_conv2d,
                                                     resnet_nonlinearity, skip_connection=1, dim_i=dim_i,
                                                     conditional=conditional, h_dim=h_dim)
                                        for _ in range(nr_resnet)])
...
class PixelCNNLayer_down(nn.Module):
    def __init__(self, nr_resnet, nr_filters, resnet_nonlinearity, dim_i=28, conditional=False, h_dim=20):
        ...
        # stream from pixels above and to thes left
        self.ul_stream = nn.ModuleList([gated_resnet(nr_filters, down_right_shifted_conv2d,
                                                     resnet_nonlinearity, skip_connection=2, dim_i=dim_i,
                                                     conditional=conditional, h_dim=h_dim)
                                        for _ in range(nr_resnet)])
...
class PixelCNN(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10,
                 resnet_nonlinearity='concat_elu', input_channels=3, dim_in=28, num_classes=1,
                 conditional=False, h_dim=20):
        ...
        up_output_dims = [dim_in] + [int(dim_in * 0.5**i) for i in range(1, 3)]
        ...
        # Claartje: I altered this to output a number of classes instead of ta logistic mixture
        # the second argument used to be num_mix * nr_logistic_mix
        self.nin_out = nin(nr_filters, num_classes)

        self.init_padding = None

    def forward(self, x, sample=False, h=None):
        if self.conditional:
            assert h is not None, "you initialised a conditional model, you need to pass conditional h"

        # similar as done in the tf repo :
        # Claartje: I changed this slightly to cope with different batch sizes
        if not sample:
            xs = [int(y) for y in x.size()]
            padding = Variable(torch.ones(xs[0], 1, xs[2], xs[3], device=x.device), requires_grad=False)
